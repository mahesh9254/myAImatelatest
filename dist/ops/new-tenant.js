...
function createAssistantCredentials(tenant) {
    ...
    return exec('./ops/conv-creds.sh')
        ...
        const credentials = dbobjects.createBluemixCredentials('conv', tenant, apikey, undefined, undefined, 'conv_standard');
...
function createVisualRecognitionCredentials(tenant) {
    ...
    return exec('./ops/visrec-creds.sh')
        ...
        const apikey = output[0];
        const notes = output[1];
        const credentials = dbobjects.createBluemixCredentials('visrec', tenant, apikey, undefined, undefined, 'visrec_standard');
        credentials.url = 'https://gateway.watsonplatform.net/visual-recognition/api';
        credentials.notes = notes;
        return store.storeBluemixCredentials(tenant, dbobjects.getCredentialsAsDbRow(credentials));
    });
}
async function createMultipleAssistantCredentials(tenant, num) {
    const credentials = [];
    for (let i = 0; i < num; i++) {
...
async function createMultipleVisualRecognitionCredentials(tenant, num) {
    const credentials = [];
    for (let i = 0; i < num; i++) {
