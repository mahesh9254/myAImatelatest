...
describe('REST API - scratch keys', () => {
    ...
    describe('get keys', () => {
        ...
        it('should return an existing trained scratch key', async () => {
            const userid = uuid();
            const project = await store.storeProject(userid, TESTCLASS, 'text', 'A Fake Project', 'en', [], false);
            const projectid = project.id;
    ...
    describe('use keys', () => {
        it('should require text to classify', async () => {
            ...
            return request(testServer)
                ...
                assert.strictEqual(res.error.text, '/**/ typeof ' + callbackFunctionName +
        ...
        it('should check the data type when classifying', async () => {
            ...
            return request(testServer)
                ...
                const classificationRespStr = text.substring(expectedStart.length, text.length - 2);
        ...
        it('should require the right number of numeric values to classify', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                await store.deleteEntireProject(userid, TESTCLASS, project);
                assert.strictEqual(res.error.text, '/**/ typeof ' + callbackFunctionName + ' === \'function\' && ' +
        ...
        function mockClassifier(url, options) {
        ...
        function brokenClassifier( /*url: string, options?: coreReq.CoreOptions*/) {
        ...
        it('should return classes from a classifier', async () => {
            ...
            const scratchKey = await store.storeOrUpdateScratchKey(project, storedCredentials, conversationWorkspace.workspace_id, conversationWorkspace.created);
            // @ts-ignore
        ...
        it('should return errors from a classifier', async () => {
            ...
            const storedCredentials = await store.storeBluemixCredentials(TESTCLASS, credentials);
            await store.storeConversationWorkspace(storedCredentials, project, workspace);
            const scratchKey = await store.storeOrUpdateScratchKey(project, storedCredentials, workspace.workspace_id, workspace.created);
            // @ts-ignore
