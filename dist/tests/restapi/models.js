...
describe('REST API - models', () => {
    ...
    before(async () => {
        ...
        conversationStubTrainClassifierStub.callsFake((project) => {
            if (project.name === 'no more room') {
                ...
                const err = new Error('Unexpected response when retrieving service credentials');
                return Promise.reject(err);
            }
            else if (project.name === 'too fast') {
                const err = new Error(conversation.ERROR_MESSAGES.API_KEY_RATE_LIMIT);
                return Promise.reject(err);
            }
            else if (project.name === 'deleted model') {
                const err = new Error(conversation.ERROR_MESSAGES.MODEL_NOT_FOUND);
                return Promise.reject(err);
            }
            else {
                const workspace = {
        ...
        numbersStubTrainClassifierStub.callsFake((project) => {
            const output = {
                created: new Date(),
                status: 'Available',
        ...
        imagesStubGetClassifiersStub.callsFake((classid, classifiers) => {
            return new Promise((resolve) => {
                let output = [];
                output = classifiers.map((classifier) => {
        ...
        imagesStubTrainClassifierStub.callsFake((project) => {
            if (project.name === 'no more room') {
                ...
                const err = new Error(visualrecog.ERROR_MESSAGES.API_KEY_RATE_LIMIT);
                return Promise.reject(err);
            }
            else {
                const workspace = {
        ...
        imagesStubTestClassifierUrlStub.callsFake((creds, classifierid, classifierTimestamp) => {
            const classifications = [
        ...
        imagesStubTestClassifierFileStub.callsFake((creds, classifierid, classifierTimestamp, projectid, imagefile) => {
    ...
    describe('newModel', () => {
        ...
        it('should handle bad text credentials in unmanaged classes', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                const body = res.body;
                assert.strictEqual(body.error, 'The Watson credentials being used by your class were rejected. ' +
        ...
        it('should handle missing text credentials in unmanaged classes', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                const body = res.body;
                assert.strictEqual(body.error, 'No Watson credentials have been set up for training text projects. ' +
                    'Please let your teacher or group leader know.');
                await store.deleteEntireUser(userid, classid);
            });
        });
        it('should handle failure when updating deleted Conversation workspaces', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                const body = res.body;
                assert.strictEqual(body.error, 'Your machine learning model could not be found on the training server. Please try again');
                await store.deleteEntireUser(userid, classid);
            });
        });
        it('should handle rate limit errors from Conversation', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                const body = res.body;
                assert.strictEqual(body.error, 'Your class is making too many requests to create machine learning models ' +
                    'at too fast a rate. ' +
        ...
        it('should handle missing images credentials in unmanaged classes', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                const body = res.body;
                assert.strictEqual(body.error, 'No Watson credentials have been set up for training images projects. ' +
                    'Please let your teacher or group leader know.');
                await store.deleteEntireUser(userid, classid);
            });
        });
        it('should handle requests to train projects without enough training data', async () => {
            ...
            return request(testServer)
                ...
                .then(async (res) => {
                const body = res.body;
                assert.strictEqual(body.error, 'Not enough images to train the classifier');
                await store.deleteEntireUser(userid, classid);
            });
        });
