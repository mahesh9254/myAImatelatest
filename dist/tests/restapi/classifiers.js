...
describe('REST API - classifiers', () => {
    ...
    const convCredentials = dbobjects.createBluemixCredentials(conv, CLASSID, undefined, username, password, 'conv_standard');
    const visrecCredentials = dbobjects.createBluemixCredentials(visrec, CLASSID, apikey, undefined, undefined, 'visrec_standard');
    const textProject = dbobjects.getProjectFromDbRow(dbobjects.createProject(USERID, CLASSID, 'text', textProjectName, 'en', [], false));
    ...
    const imagesProject = dbobjects.getProjectFromDbRow(dbobjects.createProject(USERID, CLASSID, 'images', imagesProjectName, 'en', [], false));
    ...
    describe('get unknown classifiers', () => {
        ...
        it('should return a list of unmanaged classifiers', async () => {
            ...
            return testRequest(testServer)
                ...
                assert(allUnknowns.conv.some((unknown) => unknown.name === 'First API test'));
                assert(allUnknowns.conv.some((unknown) => unknown.name === 'Second API test'));
                assert(allUnknowns.conv.some((unknown) => unknown.name === 'Third API test'));
                assert.strictEqual(allUnknowns.visrec.length, 2);
                assert(allUnknowns.visrec.some((unknown) => unknown.name === 'dogs'));
                assert(allUnknowns.visrec.some((unknown) => unknown.name === 'Cars vs Trucks'));
            });
        });
    });
    describe('delete classifiers', () => {
        let deleteClassifiersStub;
        before(async () => {
    ...
    const mockConversation = {
        ...
        deleteClassifier: (url) => {
            assert.strictEqual(url, 'https://gateway.watsonplatform.net/' +
    ...
    const mockVisRec = {
        ...
        deleteClassifier: (url) => {
            assert.strictEqual(url, 'https://gateway-a.watsonplatform.net/visual-recognition/api/v3/classifiers/' +
