...
describe('REST API - projects', () => {
    ...
    describe('getProjectsByUserId()', () => {
        ...
        function testCreateProject(classid, user, crowd) {
            return request(testServer)
        ...
        function testGetProjects(classid, user) {
            return request(testServer)
        ...
        function testSetUser(classid, user) {
            nextAuth0UserTenant = classid;
            nextAuth0UserId = user;
            nextAuth0UserRole = 'student';
        }
        function testSetTeacher(classid, user) {
            testSetUser(classid, user);
            nextAuth0UserRole = 'supervisor';
        }
        function compareProjects(objA, objB) {
            if (objA.id > objB.id) {
                return 1;
            }
            if (objA.id < objB.id) {
        ...
        async function testVerifyAll(classid, studentA, studentB, teacherId, expected) {
        ...
        it('should return crowd-sourced projects', () => {
            ...
            const studentB = uuid();
            const teacherId = uuid();
            const expectedStudentA = [];
            const expectedStudentB = [];
    ...
    describe('modifyProjectLabels()', () => {
        ...
        it('should cleanup object storage after removing a label', async () => {
            ...
            await request(testServer)
                ...
                .expect('Content-Type', /json/)
                .expect(httpstatus.OK);
            await store.storeImageTraining(projectId, '/api/classes/' + TESTCLASS +
                '/students/' + studentId +
                '/projects/' + projectId +
                '/images/' + imageIds[0], label, true, imageIds[0]);
            await store.storeImageTraining(projectId, '/api/classes/' + TESTCLASS +
                '/students/' + studentId +
                '/projects/' + projectId +
                '/images/' + imageIds[1], label, true, imageIds[1]);
            await store.storeImageTraining(projectId, '/api/classes/' + TESTCLASS +
                '/students/' + studentId +
                '/projects/' + projectId +
                '/images/' + imageIds[2], label, true, imageIds[2]);
            await store.storeImageTraining(projectId, 'http://existingimage.com/image.png', label, false);
            await request(testServer)
                .patch(url + '/' + projectId)
                .send([{
                    path: '/labels', op: 'remove',
