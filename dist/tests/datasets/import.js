...
describe('Datasets import', () => {
    ...
    let numbersTrainingServiceDeleteStub;
    before(() => {
        return store.init();
    });
    before(() => {
        const originalRequestDelete = requestPromise.delete;
        const stubbedRequestDelete = (url, opts) => {
    ...
    describe('Errors', () => {
        ...
        it('should handle requests to import non-existent dataset types', () => {
            return datasets.importDataset(uuid(), TESTCLASS, false, '../../../', 'not-a-real-dataset')
                .then(() => {
                assert.fail('should not get here');
            })
                .catch((err) => {
        ...
        it('should be able to import all prod datasets', async () => {
            ...
            for (const type of Object.keys(prodDatasets)) {
                for (const dataset of prodDatasets[type]) {
                    const project = await datasets.importDataset(user, TESTCLASS, false, type, dataset);
    ...
    describe('Text datasets', () => {
        ...
        function verifyTestTextProject(projectid) {
        ...
        function confirmItemPresent(list, textdata, label) {
    ...
    describe('Numbers datasets', () => {
        ...
        function verifyTestNumbersProject(projectid) {
            return store.getProject(projectid)
                .then((project) => {
                assert(project);
                if (project) {
    ...
    describe('Images datasets', () => {
        ...
        function verifyTestImagesProject(projectid) {
            return store.getProject(projectid)
                .then((project) => {
                assert(project);
                if (project) {
        ...
        function confirmItemPresent(list, imageurl, label) {
            return list.some((listitem) => {
