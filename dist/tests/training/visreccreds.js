...
const mockVisRec = require("./mock-visrec");
const store = require("../../lib/db/store");
describe('Training - Visual Recognition - IAM/API keys', () => {
    let getStub;
    let postStub;
    let deleteStub;
    let getClassTenantStub;
    let getImageTrainingByLabelStub;
    let countTrainingByLabelStub;
    let getImageClassifiersStub;
    let getBluemixCredentialsByIdStub;
    let getBluemixCredentialsStub;
    let storeImageClassifierStub;
    let storeScratchKeyStub;
    let resetExpiredScratchKeyStub;
    let deleteImageClassifierStub;
    let setTimeoutStub;
    let downloadStub;
    before(() => {
        iam.init();
    ...
    describe('legacy API keys', () => {
        it('should train a classifier', async () => {
            ...
            assert.strictEqual(classifier.url, mockVisRec.CREDENTIALS_LEGACY.url +
                '/v3/classifiers/' +
        ...
        it('should delete a classifier', async () => {
            ...
            assert(deleteStub.calledWith(mockVisRec.CREDENTIALS_LEGACY.url + '/v3/classifiers/' + classifierinfo.id, sinon.match.has('qs', {
                version: '2016-05-20',
    ...
    describe('current API keys', () => {
        it('should train a classifier', async () => {
            ...
            assert.strictEqual(classifier.status, 'training');
            assert.strictEqual(classifier.url, mockVisRec.CREDENTIALS_NEW.url +
                '/v3/classifiers/' +
        ...
        it('should delete a classifier', async () => {
            ...
            assert(deleteStub.calledWith(mockVisRec.CREDENTIALS_NEW.url + '/v3/classifiers/' +
                encodeURIComponent(classifierinfo.id), sinon.match.has('qs', { version: '2018-03-19' })));
            ...
            assert(setTimeoutStub.calledOnce);
            assert(deleteStub.calledWith(mockVisRec.CREDENTIALS_NEW.url + '/v3/classifiers/' +
                encodeURIComponent(classifierinfo.id), sinon.match.has('headers', sinon.match.has('Authorization', authHeader))));
        });
    });
});
