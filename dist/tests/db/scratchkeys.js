...
describe('ScratchKeys store', () => {
    ...
    beforeEach(async () => {
        project = await store.storeProject(reusedUserid, reusedClassid, 'text', randomstring.generate({ length: 20 }), 'en', [], false);
    });
    after(() => {
    ...
    it('should update an empty scratch key', async () => {
        const keyid = await store.storeUntrainedScratchKey(project);
        const retrievedEmpty = await store.getScratchKey(keyid);
        ...
        const retrievedAfter = await store.getScratchKey(keyid);
    ...
    it('should create a scratch key', async () => {
        ...
        const classifierid = randomstring.generate({ length: 12 });
        const keyid = await store.storeOrUpdateScratchKey(project, credentials, classifierid, new Date());
        return store.deleteScratchKey(keyid);
    });
    it('should retrieve a scratch key', async () => {
        ...
        const keyid = await store.storeOrUpdateScratchKey(project, credentials, classifierid, ts);
        const retrieved = await store.getScratchKey(keyid);
        delete credentials.id;
    ...
    it('should find a scratch key for a project', async () => {
        ...
        const keyid = await store.storeOrUpdateScratchKey(project, credentials, classifierid, ts);
        const response = await store.findScratchKeys(project.userid, project.id, project.classid);
    ...
    it('should update a scratch key', async () => {
        ...
        const keyid = await store.storeOrUpdateScratchKey(project, credentials, classifierid, before);
        const newClassifierId = randomstring.generate({ length: 11 });
        const after = new Date(2018, 2, 2, 0, 0, 0);
        const updatedKeyId = await store.storeOrUpdateScratchKey(project, credentials, newClassifierId, after);
    ...
    it('should reset expired scratch keys', async () => {
        ...
        const scratchKey = await store.storeOrUpdateScratchKey(project, credentials, expired.workspace_id, now);
