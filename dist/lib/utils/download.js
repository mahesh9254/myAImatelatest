 ...
 */
function file(url, targetFilePath, callback) {
    ...
    try {
        numDownloads += 1;
        request.get(Object.assign(Object.assign({}, REQUEST_OPTIONS), { url }))
            .on('response', (r) => {
            ...
            resolve(new Error(exports.ERRORS.DOWNLOAD_FAIL + url));
        })
            .pipe(writeStream);
    }
    catch (err) {
...
exports.file = file;
function recognizeCommonProblems(response, url) {
    if (response.statusCode >= 400) {
        if (response.statusCode === httpstatus.FORBIDDEN ||
            response.statusCode === httpstatus.UNAUTHORIZED) {
            return new Error(safeGetHost(url) + exports.ERRORS.DOWNLOAD_FORBIDDEN);
        }
        numErrors += 1;
        log.error({ statusCode: response.statusCode, url, numDownloads, numErrors }, 'Failed to request url');
        return new Error(exports.ERRORS.DOWNLOAD_FAIL + url);
    }
    if (downloadTooBig(response.headers)) {
        return new Error(exports.ERRORS.DOWNLOAD_TOO_BIG);
    }
    if (response.headers['content-type'] &&
        response.headers['content-type'].startsWith('text/html') &&
        response.request.uri.href.startsWith('https://accounts.google.com/ServiceLogin?continue=')) {
        return new Error('Google' + exports.ERRORS.DOWNLOAD_FORBIDDEN);
    }
}
/**
 ...
 */
function downloadTooBig(headers) {
...
exports.downloadTooBig = downloadTooBig;
/**
 * Downloads a file from the specified URL to the specified location on disk.
 *
 * @param url  - downloads from
 ...
 * @param targetFilePath  - writes to
 */
function resize(url, width, height, targetFilePath, callback) {
    // local inner function used to avoid calling callback multiple times
    let resolved = false;
    function resolve(err) {
        if (resolved === false) {
    ...
    probe(url, { rejectUnauthorized: false })
        ...
        if (err.statusCode === httpstatus.NOT_FOUND || err.message === 'ETIMEDOUT') {
            ...
            return resolve(new Error(safeGetHost(url) + exports.ERRORS.DOWNLOAD_FORBIDDEN));
        }
        else {
