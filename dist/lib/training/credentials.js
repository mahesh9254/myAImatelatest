...
const bluemixclassifiers = require("./classifiers");
 ...
 * references to them.
 */
async function deleteBluemixCredentials(credentials) {
...
async function checkBluemixCredentials() {
    log.info('Checking Bluemix credentials');
    slack.notify('Checking Bluemix credentials', slack.SLACK_CHANNELS.CREDENTIALS);
...
exports.checkBluemixCredentials = checkBluemixCredentials;
function reportMissingErrors(expectedErrors) {
    if (expectedErrors.badBluemixConvCredentials.length > 0) {
        slack.notify('Failed to find expected bad conv credentials : ' +
            expectedErrors.badBluemixConvCredentials.join(', '), slack.SLACK_CHANNELS.CREDENTIALS);
    }
    if (expectedErrors.badBluemixVisrecCredentials.length > 0) {
        slack.notify('Failed to find expected bad visrec credentials : ' +
            expectedErrors.badBluemixVisrecCredentials.join(', '), slack.SLACK_CHANNELS.CREDENTIALS);
    }
    if (expectedErrors.unmanagedConvClassifiers.length > 0) {
        slack.notify('Failed to find expected conv classifiers : ' +
            expectedErrors.unmanagedConvClassifiers.join(', '), slack.SLACK_CHANNELS.CREDENTIALS);
    }
    if (expectedErrors.unmanagedVisrecClassifiers.length > 0) {
        slack.notify('Failed to find expected visrec classifiers : ' +
            expectedErrors.unmanagedVisrecClassifiers.join(', '), slack.SLACK_CHANNELS.CREDENTIALS);
    }
}
async function reportBadCredentials(err, credentials) {
    ...
    slack.notify('Test for ' + servicename +
        ...
        ' failed with error : ' + err.message, slack.SLACK_CHANNELS.CREDENTIALS);
    //
    ...
    await store.storeNewKnownError(training_types_1.KnownErrorCondition.BadBluemixCredentials, credentials.servicetype, credentials.id);
    // send an email
...
async function reportUnmanagedClassifier(classifier, creds) {
    ...
    slack.notify('Unmanaged Bluemix classifier detected! ' +
        ...
        ' id:' + classifier.id +
        ' creds: ' + creds.id +
        ' class: ' + creds.classid, slack.SLACK_CHANNELS.CREDENTIALS);
    //
    // Notify the teacher responsible for the class
    //
    // store the report, so we don't report this multiple times
    await store.storeNewKnownError(training_types_1.KnownErrorCondition.UnmanagedBluemixClassifier, creds.servicetype, classifier.id);
    // send an email
 ...
 */
function isErrorExpected(expectedErrors, errortype, servicetype, id) {
...
async function handleUnknownClassifier(credentials, classifier) {
    ...
    if (classPolicy.isManaged) {
        ...
        slack.notify('DELETING UNKNOWN ' + classifier.type + ' classifier ' +
            ' id : ' + classifier.id +
            ' name : ' + classifier.name +
            ' from class ' + credentials.classid, slack.SLACK_CHANNELS.CREDENTIALS);
