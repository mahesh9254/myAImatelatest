...
const wikimedia = require("../utils/wikimedia");
const downloadAndZip = require("../utils/downloadAndZip");
...
function getMaxImageFileSize() {
    return MAX_IMAGE_FILE_SIZE_BYTES;
}
exports.getMaxImageFileSize = getMaxImageFileSize;
//
...
async function createClassifier(project, credentialsPool, training) {
    let classifier;
    ...
    for (const credentials of credentialsPool) {
        try {
            ...
            if (err.error &&
                ...
                log.warn({ err, project, credentials: credentials.id }, 'Credentials rejected');
                throw err;
            }
            else {
                ...
                notifications.notify('Unexpected failure to train image classifier' +
                    ...
                    err.message + detail, notifications.SLACK_CHANNELS.TRAINING_ERRORS);
                throw err;
            }
        }
    }
 ...
 */
function getClassifierStatuses(classid, classifiers) {
    const credentialsCacheById = {};
    return Promise.all(classifiers.map(async (classifier) => {
...
async function getStatus(credentials, classifier) {
    let req;
    try {
 ...
 */
function validateRequest(locations) {
...
async function getImageDownloadSpec(imageid, imageurl) {
 ...
 * @param classifierid
 */
function deleteClassifierFromBluemixAgain(credentials, classifierId) {
    setTimeout(() => {
        deleteClassifierFromBluemix(credentials, classifierId)
            ...
            notifications.notify('Failed to delete Visual Recognition classifier ' +
                ...
                'Model : ' + classifierId + '\n' +
                'Creds : ' + credentials.id, notifications.SLACK_CHANNELS.TRAINING_ERRORS);
        });
...
async function testClassifierFile(credentials, classifierid, classifierTimestamp, projectid, imagefilepath) {
    ...
    const req = Object.assign(Object.assign({}, basereq), testreq);
    ...
    try {
        ...
        if (err.error &&
            ...
            if (classifierNotFoundError(errorInfo)) {
                const externalError = new Error(exports.ERROR_MESSAGES.NO_MODEL);
                ...
                const externalError = new Error('Invalid image data provided. ' +
                ...
                const externalError = new Error('Image is too small to be recognized');
                externalError.statusCode = 400;
                throw externalError;
            }
            else if (errorInfo.code === 500 &&
                errorInfo.description === 'Internal error performing classification') {
                const externalError = new Error('The IBM Watson Visual Recognition service ' +
            ...
            const externalError = new Error('Missing data');
            externalError.statusCode = 400;
            throw externalError;
        }
        throw err;
...
exports.testClassifierFile = testClassifierFile;
async function testClassifierURL(credentials, classifierid, classifierTimestamp, projectid, imageurl) {
    const basereq = await createBaseRequest(credentials);
    const testreq = {
        qs: {
            url: imageurl,
    ...
    const req = {
        qs: Object.assign(Object.assign({}, basereq.qs), testreq.qs),
    ...
    try {
        const url = credentials.url + '/v3/classify';
        const body = await request.get(url, req);
        if (body.images &&
            body.images.length > 0 &&
            body.images[0].classifiers &&
            body.images[0].classifiers.length > 0 &&
        ...
        if (err.error &&
            ...
            if (errorInfo.code === 400 &&
                errorInfo.description.startsWith('Invalid/corrupted image data. ' +
                    'Supported image file formats are GIF, JPEG, PNG, and TIFF')) {
                const externalError = new Error('A usable test image could not be found at that address. ' +
                    'Remember, only jpg and png images are supported.');
                externalError.statusCode = 400;
                throw externalError;
            }
                ...
                const externalError = new Error('Web address could not be contacted. ' +
                ...
                const externalError = new Error('Invalid URL. ' +
                    'Please enter the web address for a picture that you want to ' +
                    'test your machine learning model on');
                externalError.statusCode = 400;
                throw externalError;
                ...
                const externalError = new Error('Access forbidden. ' +
...
exports.testClassifierURL = testClassifierURL;
function classifierNotFoundError(errorInfo) {
    return errorInfo &&
        errorInfo.code &&
        errorInfo.code === 404 &&
...
function sortByConfidence(item1, item2) {
    return item2.confidence - item1.confidence;
}
async function submitTrainingToVisualRecognition(project, credentials, url, training, tenantPolicy) {
    const trainingData = {
        name: project.name,
    };
    for (const label of project.labels) {
    ...
    const basereq = await createBaseRequest(credentials);
    const req = Object.assign(Object.assign({}, basereq), { formData: trainingData });
    try {
        const body = await request.post(url, req);
        ...
        log.warn({ url, req, project, err }, exports.ERROR_MESSAGES.UNKNOWN);
        ...
        if (err.statusCode === httpStatus.REQUEST_ENTITY_TOO_LARGE) {
            try {
                ...
                const credsCheckError = new Error(exports.ERROR_MESSAGES.UNKNOWN);
...
async function getImageClassifiers(credentials) {
    const req = await createBaseRequest(credentials);
    // GETs don't need as long a timeout as POSTs
    req.timeout = 30000;
 ...
 */
async function identifyRegion(credentials) {
...
async function cleanupExpiredClassifiers() {
    ...
    const expired = await store.getExpiredImageClassifiers();
...
async function createBaseRequest(credentials) {
    ...
    const api_key = credentials.username + credentials.password;
    if (getType(credentials) === 'legacy') {
        const req = {
            qs: {
                version: '2016-05-20',
