...
function returnConversationWorkspace(classifier) {
    return {
...
function transformStatus(visualClassifierStatus) {
...
function returnVisualRecognition(classifier) {
    return {
...
function returnNumberClassifier(classifier) {
...
async function newModel(req, res) {
    switch (req.project.type) {
        case 'text': {
            try {
...
async function describeModel(req, res) {
...
function registerApis(app) {
    app.get(urls.MODELS, auth.authenticate, auth.checkValidUser, auth.verifyProjectAccess, 
    // @ts-ignore
    getModels);
    app.post(urls.MODELS, auth.authenticate, auth.checkValidUser, auth.verifyProjectOwner, 
    // @ts-ignore
    newModel);
    app.get(urls.MODEL, auth.authenticate, auth.checkValidUser, auth.verifyProjectAccess, 
    // @ts-ignore
    describeModel);
    app.delete(urls.MODEL, auth.authenticate, auth.checkValidUser, auth.verifyProjectOwnerOrTeacher, 
    // @ts-ignore
    deleteModel);
    app.post(urls.MODELTEST, auth.authenticate, auth.checkValidUser, testModel);
}
