...
const Objects = require("../db/db-types");
const dbobjects = require("../db/objects");
const datasets = require("../datasets");
const users = require("../auth0/users");
...
function getProjectsByClassId(req, res) {
...
async function createProject(req, res) {
    ...
    try {
        let project;
        if (isImport) {
            project = await datasets.importDataset(userid, classid, crowdsourced, req.body.type, req.body.dataset);
        }
        else {
            project = await store.storeProject(userid, classid, req.body.type, req.body.name, req.body.language, req.body.fields, crowdsourced);
        }
...
async function deleteImages(classid, userid, projectid, label) {
...
function registerApis(app) {
    app.get(urls.ALL_CLASS_PROJECTS, auth.authenticate, auth.checkValidUser, auth.requireSupervisor, getProjectsByClassId);
    app.get(urls.PROJECTS, auth.authenticate, auth.checkValidUser, getProjectsByUserId);
    app.post(urls.PROJECTS, auth.authenticate, auth.checkValidUser, 
    // @ts-ignore
    createProject);
    app.get(urls.PROJECT, auth.authenticate, auth.checkValidUser, auth.verifyProjectAccess, 
    // @ts-ignore
    getProject);
    app.get(urls.FIELDS, auth.authenticate, auth.checkValidUser, getProjectFields);
    app.delete(urls.PROJECT, auth.authenticate, auth.checkValidUser, auth.verifyProjectOwner, 
    // @ts-ignore
    deleteProject);
    app.patch(urls.PROJECT, auth.authenticate, auth.checkValidUser, auth.verifyProjectOwner, modifyProject);
}
