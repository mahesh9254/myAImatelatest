...
const dbobjects = require("../db/objects");
const conversation = require("../training/conversation");
const visualRecognition = require("../training/visualrecognition");
const credentialsmgr = require("../training/credentials");
const urls = require("./urls");
const errors = require("./errors");
const logger_1 = require("../utils/logger");
const log = logger_1.default();
function returnVisualRecognitionCredentials(credentials) {
    return {
        id: credentials.id,
...
function returnConversationCredentials(credentials) {
...
async function addCredentials(req, res) {
    ...
    let newCredentials;
    try {
        newCredentials = dbobjects.createBluemixCredentials(req.body.servicetype, tenant, req.body.apikey, req.body.username, req.body.password, req.body.credstype);
    }
    catch (err) {
    ...
    try {
        ...
        res.status(httpstatus.CREATED).json(credsObj.servicetype === 'visrec' ?
...
async function modifyCredentials(req, res) {
    const tenant = req.params.classid;
    const credsid = req.params.credentialsid;
    let patch;
    try {
...
function getCredentialsPatch(req) {
...
function registerApis(app) {
    app.get(urls.BLUEMIX_CREDENTIALS, auth.authenticate, auth.checkValidUser, auth.requireSupervisor, auth.ensureUnmanaged, getCredentials);
    app.delete(urls.BLUEMIX_CREDENTIAL, auth.authenticate, auth.checkValidUser, auth.requireSupervisor, auth.ensureUnmanaged, deleteCredentials);
    app.patch(urls.BLUEMIX_CREDENTIAL, auth.authenticate, auth.checkValidUser, auth.requireSupervisor, auth.ensureUnmanaged, modifyCredentials);
    app.post(urls.BLUEMIX_CREDENTIALS, auth.authenticate, auth.checkValidUser, auth.requireSupervisor, auth.ensureUnmanaged, addCredentials);
}
